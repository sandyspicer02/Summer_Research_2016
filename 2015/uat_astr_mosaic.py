...
NOTES:
sextractor creates *.cat files.  these are FITS_LDAC files and contain the basic WCS information for each image.  To access the catalogs in python:

ipython -pylab

> from astropy.io import fits
infile='hftr7134t0027o00.cat'
d2=fits.getdata(infile,2)
d2.columns
figure()
plot(d2['X_IMAGE'],d2['Y_IMAGE'],'bo')
...
parser = argparse.ArgumentParser(description ='Run sextractor, scamp, and swarp to determine WCS solution and make mosaics.  For example \n \
/home/share/research/pythonCode/uat_astr_mosaic.py --filestring "hftr*.fits" --s \n')
...
parser.add_argument('--d', dest = 'd', default = '/home/share/research/astromatic/', action = 'store_true', help = 'Directory containing default.* files for sextractor, scamp and swarp.  To use current directory enter "" ')

parser.add_argument('--c', dest = 'c', default = False, action = 'store_true', help = 'Run scamp')
parser.add_argument('--w', dest = 'w', default = False, action = 'store_true', help = 'Run swarp for each object/filter combination to create mosaics')
