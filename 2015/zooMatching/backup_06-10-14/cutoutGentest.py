...
hight=input("What do you want the hight of the cut out to be? ")
width=input("What do you want the width of the cut out to be? ")
...
def makecuts():
        ...
        for i in range(len(zFlag)):
            if (zFlag[i] & onimageflag[i]):
                cutout(image, fdata.RA[i], fdata.DEC[i],xw=width ,yw=hight, outfile=outimage  +clustername + fdata.NSAID[i]+".fits")
